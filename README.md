...
| **학교생활 에이전트** (Beta) | 학과·학년·목표·시간표를 주면 에이전트가 **실시간 웹검색**으로 공모전·행사·자격증·커리큘럼·면접 대비를 한 번에 브리핑. 출처와 대조 검증된 항목엔 `✓ 출처 확인됨` 배지, 마감이 있으면 `D-7` 표시. 마음에 든 항목은 "내 플랜"에 저장해 **마감 임박순**으로 상태(예정/진행/완료) 관리 |
...
flowchart LR
    ...
    BE -->|부팅 시 캐시 로드| DB
    BE -->|파싱·검색 루프·합성·근거검증| SOLAR["Upstage Solar<br/>(open2 + pro3 + mini)"]
...
- **에이전트 = 모델 믹스**: 학교생활 에이전트는 클로드식 검색 루프의 미니어처다 — Solar가 function calling으로 `web_search` 도구(Naver)를 스스로 병렬 호출(최대 3라운드)하고, 수집된 근거만으로 브리핑을 합성한 뒤, 만들어진 항목을 다시 출처와 대조한다.

단계별로 성격이 다른 작업이라, 역할마다 다른 Solar 모델을 쓴다:

| 단계 | 모델 | 이유 |
|---|---|---|
| 검색 판단 (function calling) | `solar-open2` + `reasoning_effort: minimal` | 무엇을 검색할지 정하는 가벼운 추론 |
| 브리핑 합성 (JSON) | `solar-pro3` | open2는 대량 출력이 느려 합성에서 60초+ 소요(실측) |
| 근거 검증 (groundedness) | `solar-mini` | 대조는 단순 작업 — 큰 모델을 써도 정확도는 같고 느려지기만 했다(건당 0.4초 vs 1.2초) |

이 역할 분담으로 실행 시간을 **139초→52초**로 줄였다(실측). 검증은 항목별 병렬 호출이라 +1초 수준이다.
...
| AI | Upstage Solar — `solar-open2`(선호 파싱·에이전트 검색 루프, function calling) + `solar-pro3`(브리핑 합성) + `solar-mini`(근거 검증), JSON mode |
